...
**You MUST verify current codebase state before EACH AND EVERY PHASE. Use `morphe:codebase-investigator` to prove out your hypotheses and to ensure that current state aligns with what you want to write out.**

**YOU MUST verify current codebase state before writing ANY task.**

**DO NOT verify codebase yourself. Use morphe:codebase-investigator agent.**

**Provide the agent with design assumptions so it can report discrepancies:**

Dispatch one subagent morphe:codebase-investigator to understand testing behavior for this project.
...
Dispatch a second subagent morphe:codebase-investigator (simultaneously) with:
...
Use `morphe:internet-researcher` for:
...
Use `morphe:remote-code-researcher` when:
...
├─ No → morphe:codebase-investigator only
└─ Yes → What do we need to know?
    ├─ API usage, standard patterns → morphe:internet-researcher
    ├─ Standard/spec implementation → morphe:internet-researcher
    ├─ Implementation internals, extension points → morphe:remote-code-researcher
    └─ Both local state + external info → morphe:combined-researcher
```
...
**Standards vs Implementation:** Standards questions (e.g., "how does OAuth2 work") are morphe:internet-researcher territory. Implementation questions (e.g., "how does auth0-js store tokens") may require morphe:remote-code-researcher.
...
- `DESIGN_PATH`: Absolute path to design plan (e.g., `/Users/ed/project/.techne/tasks/2025-01-24-feature.md`)
- `PLAN_DIR`: Absolute path to implementation plan directory (e.g., `/Users/ed/project/.techne/tasks/2025-01-24-feature/`)
...
Before creating the Finalization task, check if `.techne/implementation-plan-guidance.md` exists. If it does, include its absolute path in the task description:

```markdown
# If .techne/implementation-plan-guidance.md exists:
- [ ] Finalization: Run morphe:code-reviewer over all phase files (guidance: [absolute path to .techne/implementation-plan-guidance.md]), fix ALL issues including minor ones
      → blocked by: all Phase *D tasks

# If .techne/implementation-plan-guidance.md does NOT exist:
- [ ] Finalization: Run morphe:code-reviewer over all phase files, fix ALL issues including minor ones
      → blocked by: all Phase *D tasks
```

**Example for a 3-phase design at `/Users/ed/project/.techne/tasks/2025-01-24-oauth.md`:**

```
TaskCreate: "Phase 1A: Read Token Types from /Users/ed/project/.techne/tasks/2025-01-24-oauth.md"
...
TaskCreate: "Phase 1D: Write /Users/ed/project/.techne/tasks/2025-01-24-oauth/phase_01.md"
  → TaskUpdate: addBlockedBy: [1C]

TaskCreate: "Phase 2A: Read Token Service from /Users/ed/project/.techne/tasks/2025-01-24-oauth.md"
...
TaskCreate: "Phase 2D: Write /Users/ed/project/.techne/tasks/2025-01-24-oauth/phase_02.md"
  → TaskUpdate: addBlockedBy: [2C]

TaskCreate: "Phase 3A: Read Session Manager from /Users/ed/project/.techne/tasks/2025-01-24-oauth.md"
...
TaskCreate: "Phase 3D: Write /Users/ed/project/.techne/tasks/2025-01-24-oauth/phase_03.md"
  → TaskUpdate: addBlockedBy: [3C]

TaskCreate: "Finalization: Run morphe:code-reviewer over all phase files, fix ALL issues including minor ones"
...
2. **Task NB: Verify codebase state**
   ...
   - Dispatch morphe:codebase-investigator with design assumptions for this phase
...
3. **Task NC: Research external dependencies** (if phase involves them)
   ...
   - Dispatch morphe:internet-researcher for docs/standards/API patterns
   - Escalate to morphe:remote-code-researcher if docs are insufficient
...
7. **Task ND: Write phase file (if approved)**
   ...
   - Write to `.techne/tasks/YYYY-MM-DD-<feature-name>/phase_##.md`
...
4. **Task ND: Write phase file**
   ...
   - Write directly to disk at `.techne/tasks/YYYY-MM-DD-<feature-name>/phase_##.md`
   - Mark task ND as completed, continue to next phase
...
Announce: "All [N] phase files written to `.techne/tasks/YYYY-MM-DD-<feature-name>/`. Let me know if any phases need revision."
...
| Excuse | Reality |
|--------|---------|
| "File probably exists, I'll say 'update if exists'" | Use morphe:codebase-investigator. Write definitive instruction. |
| "Design mentioned this file, must be there" | Codebase changes. Use investigator to verify current state. |
| "I can quickly verify files myself" | Use morphe:codebase-investigator. Saves context and prevents hallucination. |
...
| "I know how this library works from training" | Research it. APIs change. Use morphe:internet-researcher for docs, morphe:remote-code-researcher for internals. |
| "Docs are probably accurate enough" | Usually yes. But if extending/customizing library behavior, verify with source code. |
| "I'll clone the repo to check the docs" | No. Use morphe:internet-researcher for docs. Only clone (morphe:remote-code-researcher) for source code investigation. |
...
2. **Context about why this blocks you:**
   ...
   - What you've already verified via morphe:codebase-investigator
...
- [ ] **Task NB:** Mark in_progress, dispatch morphe:codebase-investigator, review findings, mark completed
...
- [ ] Dispatch morphe:code-reviewer to validate plan against design
- [ ] Fix ALL issues including Minor ones
- [ ] Re-run morphe:code-reviewer until APPROVED with zero issues
...
**This is a tracked task: "Finalization: Run morphe:code-reviewer over all phase files, fix ALL issues including minor ones"**

After all phase D tasks are completed, mark the Finalization task as in_progress.

### Step 1: Dispatch morphe:code-reviewer
...
<parameter name="prompt">
  ...
  DESIGN_PLAN: [path to design plan, e.g., .techne/tasks/YYYY-MM-DD-feature.md]

  IMPLEMENTATION_GUIDANCE: [absolute path to .techne/implementation-plan-guidance.md, or "None" if file does not exist]
...
6. Re-run morphe:code-reviewer validation
...
**Only when morphe:code-reviewer returns APPROVED with zero issues:**
...
```
<invoke name="Task">
<parameter name="subagent_type">morphe:opus-general-purpose</parameter>
...
**"Implementation plan complete and validated. Saved to [count] phase files + test-requirements.md in `.techne/tasks/YYYY-MM-DD-<feature-name>/`. The first phase file is `<full-path>`. Test requirements are in `<full-path>/test-requirements.md`."**
