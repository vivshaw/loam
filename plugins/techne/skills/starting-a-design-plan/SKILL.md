...
- (conditional) Read project design guidance (if `.techne/design-plan-guidance.md` exists)
...
- Phase 5: Design Documentation (design written to .techne/tasks/)
...
**Check if `.techne/design-plan-guidance.md` exists:**

Use the Read tool to check if `.techne/design-plan-guidance.md` exists in the session's working directory.

**If the file exists:**

1. Use TaskCreate to add: "Read project design guidance from [absolute path to .techne/design-plan-guidance.md]"
...
**File location:** `.techne/tasks/YYYY-MM-DD-{slug}/spec.md`
...
```
Design complete! Design document committed to `.techne/tasks/<slug>/spec.md`.
...
```
/techne:plan @.techne/tasks/<slug>/spec.md
```
...
The /techne:plan command will create detailed tasks, set up a branch, and prepare for execution.
```
