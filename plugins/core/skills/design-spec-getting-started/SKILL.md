---
name: design-spec-getting-started
description: Use when beginning any design process.
...
Orchestrate the complete design workflow from initial idea to implementation-ready documentation through six structured phases: context gathering, clarification, definition of done, brainstorming, design spec, and planning handoff.
...
**Announce at start:** "I'm using the `core:design-spec-getting-started` skill to guide us through the design process."
...
- Phase 5: Design Spec (design written to .loam/tasks/)
- Phase 6: Planning Handoff (project plan offered/created)
...
**Check if `.loam/design-spec-guidance.md` exists:**

Use the Read tool to check if `.loam/design-spec-guidance.md` exists in the session's working directory.

**If the file exists:**

1. Use TaskCreate to add: "Read project design guidance from [absolute path to .loam/design-spec-guidance.md]"
...
```
Question: "What should we call this design spec? The name becomes the prefix for all acceptance criteria (e.g., `{slug}.AC1.1`) and appears in test names.
...
**File location:** `.loam/tasks/YYYY-MM-DD-{slug}/spec.md`
...
- Acceptance Criteria, Summary, and Glossary filled in later by `core:design-spec-writing` skill
...
- This reduces Phase 1 of `core:design-spec-brainstorming` (Understanding) since much is already known

The `core:design-spec-brainstorming` skill will:
...
**Important:** The design spec already exists from Phase 3 with:
- Title
- Summary placeholder
...
After design is documented, guide user to create project plan in fresh context.
...
That skill will create detailed tasks, set up a branch, and prepare for execution.
```

**Why /clear instead of continuing:**
- Project planning needs fresh context for codebase investigation
...
- Design spec work reveals missing details -> Return to Phase 4
