...
description: Use when completing tasks, implementing major features, or before merging to verify work meets requirements - dispatches core:critic-code-reviewer subagent, handles retries and timeouts, manages review-fix loop until zero issues
...
**If the calling context provides a SCRATCHPAD_DIR, pass it to core:critic-code-reviewer.**

This prevents collisions when multiple planning/execution sessions run in parallel. The SCRATCHPAD_DIR is a namespaced temp directory (e.g., `/tmp/plan-2025-01-24-feature-a7f3b2/`) that the core:critic-code-reviewer uses for any scratch files.
...
**Dispatch `core:critic-code-reviewer` subagent:**
...
**Called by:**
- `core:execute-implement-a-project` (after each task)
- `core:execute-finishing-a-development-branch` (final review)
- Ad-hoc when you need a review

**Template location:** critique-reviewing-code/code-reviewer.md
