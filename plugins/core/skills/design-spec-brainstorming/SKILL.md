---
name: design-spec-brainstorming
description: Use during design-spec work to refine a rough idea into a validated design. Structured Socratic questioning, alternative exploration, and incremental validation.
...
**Announce at start:** "I'm using the `core:design-spec-brainstorming` skill to refine your idea into a design."
...
**Use core:researcher-codebase when you need to:**
...
Action: Dispatch core:researcher-codebase with: "Find authentication implementation, including file locations, patterns used, and dependencies"
```

### When to Use core:researcher-internet

**Dispatch the `core:researcher-internet` agent for thorough research. WebSearch/WebFetch directly is fine for quick lookups.**
...
Action: Dispatch core:researcher-internet with: "Find current best practices for file uploads in [framework], including official docs and common patterns"
```
...
- Don't research project-specific code (use core:researcher-codebase)
...
1. Use core:researcher-codebase to find it
...
1. **Investigate current state** - DON'T do this yourself:
   - Dispatch core:researcher-codebase to verify project structure
...
- "Where is auth implemented?" -> `core:researcher-codebase`
- "What auth library should we use?" -> `core:researcher-internet` (if not in codebase)
...
**If `core:design-spec-getting-started` already gathered context:**
...
1. **Research existing patterns** - DON'T do this yourself:
   - Dispatch `core:researcher-codebase`: "Find similar features and patterns used"
...
- **Implementation code: NOT OK.** Function bodies, algorithms, and executable logic belong in project plans, not design.

The distinction: contracts define boundaries between components. Implementation defines behavior within components. Brainstorming validates the boundaries; project planning fills in the behavior.
...
**Use core:researcher-codebase for:**
...
**Next step:** The orchestrating skill (`core:design-spec-getting-started`) will invoke `core:design-spec-writing` to document this design.

**You do NOT:**
- Write design spec (that's `core:design-spec-writing`)
- Set up worktrees (that's later in workflow)
- Create project plans (that's `core:project-writing-plan`)
