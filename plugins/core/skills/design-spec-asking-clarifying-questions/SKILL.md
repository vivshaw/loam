---
name: design-spec-asking-clarifying-questions
description: Use when filling in context from the user, after initial design context is gathered, before brainstorming.
...
**Announce at start:** "I'm using the `core:design-spec-asking-clarifying-questions` skill to make sure I understand your requirements correctly."
...
- Asking for initial requirements (that's `core:design-spec-getting-started` Phase 1)

## Before Clarifying

Try to answer your own questions and disambiguate from the context of the working directory. Use available subagents, such as `core:researcher-codebase`, to explore for existing work that can help explain the the subject under clarification. When you recognize elements such as common technologies or proper nouns, use `core:researcher-combined` instead to synthesize both the codebase and internet searches.

You may have other skills or MCPs containing useful information, such as connections to remote datastores used for product management purposes. You should send out `core:general-purpose-haiku` subagents to investigate them when they're appropriate.
...
2. **If you have a firm grasp**, state it back and confirm:
   ...
   Options:
   - "Yes, that's right" (proceed to `core:design-spec-brainstorming`)
...
**The Definition of Done becomes part of the output bundle** and will appear prominently at the top of the final design spec.
...
- Dispatch `core:researcher-codebase`: "Is library X already in use?"
...
This bundle gives `core:design-spec-brainstorming` a concrete, unambiguous starting point.
...
Stop and move to `core:design-spec-brainstorming` when:
...
```
Context Gathering (`core:design-spec-getting-started` Phase 1)
...
Brainstorming (`core:design-spec-getting-started` Phase 3)
  -> Explore: Architecture options, library choices, implementation phases
  -> Uses clarified requirements as foundation
```

**Purpose:** Ensure `core:design-spec-brainstorming` builds the right thing, not the wrong thing well.
